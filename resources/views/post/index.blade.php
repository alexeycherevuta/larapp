...
<div class="container mx-auto leading-normal">
    <header>
        ...
        <p class="float-right mt-3">
            <a href="{{ route('post.create') }}" class="bg-blue hover:bg-blue-dark text-white py-2 px-4 rounded  no-underline">Rédiger une news</a>
        </p>
        @endcan
        ...
        <div class="w-full md:w-1/2 lg:w-1/2 px-4 mb-8">
            <div class="border border-grey-light bg-white shadow">
                ...
                <div class="px-6 py-4 text-right">
                    @can('edit-articles')
                        <a href="{{ route('post.edit', $post->id) }}" class="inline-block bg-transparent hover:bg-grey-lighter text-grey-dark hover:text-grey-darker py-2 px-4 border hover:border-grey rounded-sm no-underline leading-tight">
                            Edit
                        </a>
                    @endcan
                    @can('delete-articles')
                    <form method="post" action="{{ route('post.destroy', $post->id) }}" class="inline-block">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
