...
<div class="container mx-auto leading-normal">
    ...
    <form class="w-full" method="post" action="{{ route('post.update',  $post->id) }}">
        {{ method_field('PUT') }}
        {{ csrf_field() }}
        <div class="mb-6">
        ...
        <div class="mb-6">
            ...
            <div class="w-full block">
                <textarea id="blog-content" name="body" cols="30" rows="10" class="bg-white appearance-none border-2 border-grey-light hover:border-blue rounded w-full py-2 px-4 text-grey-darker">{{ old('body', $post->body) }}</textarea>
            </div>
        </div>
